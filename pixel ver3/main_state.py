...
class Turret:
    ...
    def update(self,frame_time):
        self.launchTime +=frame_time*130
        self.frame = (self.frame+frame_time*self.frameSpeed)%25
        if self.launchTime>300:
...
def enter():
    ...
    Sound_Manager.PlayRepeatSound('Battle')
...
def update():
    ...
    MTime +=frame_time*130
    MakeTime +=frame_time*130

    if monstercount<3:
        if(MakeTime<4000): #실수로 하면 안되는데 자꾸 실수로나옴... 어떻게하징?
            if MTime>300:
                monsterSet.append(Monster(monstercount))
                MTime=0
        if (len (monsterSet) == 0 and MakeTime>3000):
            monstercount +=1
            MakeTime=0
    else:
        if(MakeTime>4000):
